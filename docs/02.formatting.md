...
If you are working with images you may quickly fetch `Intervention\Image\Image` object:

```php
    $img = $source->image(); // returns Image object
    $img->rotate(-45)->save($tmpPath); // manipulate and save to temp file
```
