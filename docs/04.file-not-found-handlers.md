...
$exception->getFormat(); // returns the name of not found format or null (if origin was not found)
...
- instance of \Bicycle\FilesManager\Contracts\FileSource: handlers process will be stopped,
    this file source (already with `$format == null`) will be used to get value.
